...
var fs__default = /*#__PURE__*/_interopDefaultLegacy(fs$k);
...
var EventEmitter$1 = require$$0__default$3["default"].EventEmitter;
var ReadStream = fs__default["default"].ReadStream;
var Stream$2 = require$$2__default["default"];
...
(function (module) {
	...
	const Stream = require$$2__default["default"];
...
function formatLocation$2 (callSite) {
  return relative$2(basePath$3, callSite[0]) +
    ':' + callSite[1] +
    ':' + callSite[2]
}
...
var httpAssert = assert$3;

function assert$3 (value, status, msg, opts) {
  if (value) return
  throw createError$2(status, msg, opts)
}

assert$3.fail = function (status, msg, opts) {
  assert$3(false, status, msg, opts);
};

assert$3.equal = function (a, b, status, msg, opts) {
  assert$3(a == b, status, msg, opts); // eslint-disable-line eqeqeq
};

assert$3.notEqual = function (a, b, status, msg, opts) {
  assert$3(a != b, status, msg, opts); // eslint-disable-line eqeqeq
};

assert$3.ok = function (value, status, msg, opts) {
  assert$3(value, status, msg, opts);
};

assert$3.strictEqual = function (a, b, status, msg, opts) {
  assert$3(a === b, status, msg, opts);
};

assert$3.notStrictEqual = function (a, b, status, msg, opts) {
  assert$3(a !== b, status, msg, opts);
};

assert$3.deepEqual = function (a, b, status, msg, opts) {
  assert$3(eql(a, b), status, msg, opts);
};

assert$3.notDeepEqual = function (a, b, status, msg, opts) {
  assert$3(!eql(a, b), status, msg, opts);
};
...
function formatLocation$1 (callSite) {
  return relative$1(basePath$2, callSite[0]) +
    ':' + callSite[1] +
    ':' + callSite[2]
}
...
(function (module) {
	...
	const qs = require$$5__default$1["default"];
...
const debug$4 = src$1.exports('koa:application');
...
const Stream$1 = require$$2__default["default"];
...
application.exports = class Application extends Emitter {
  ...
  constructor(options) {
    ...
    if (util$3.inspect.custom) {
      this[util$3.inspect.custom] = this.inspect;
    }
    if (options.asyncLocalStorage) {
      const { AsyncLocalStorage } = require$$17__default["default"];
      assert$2(AsyncLocalStorage, 'Requires node 12.17.0 or higher to enable asyncLocalStorage');
  ...
  use(fn) {
    ...
    debug$4('use %s', fn._name || fn.name || '-');
  ...
  onerror(err) {
    ...
    const isNativeError =
      Object.prototype.toString.call(err) === '[object Error]' ||
      err instanceof Error;
    if (!isNativeError) throw new TypeError(util$3.format('non-error thrown: %j', err));
...
var Koa = application.exports;
application.exports.HttpError;
...
function resolvePath$1 (rootPath, relativePath) {
  ...
  if (path.indexOf('\0') !== -1) {
    throw createError$1(400, 'Malicious Path')
  }
...
const fs$j = fs__default["default"];
const util$2 = require$$1__default["default"];
const debug$3 = src$1.exports('koa-send');
const resolvePath = resolvePath_1;
const createError = httpErrors$1.exports;
const assert$1 = require$$5__default["default"];

const stat$5 = util$2.promisify(fs$j.stat);
const access = util$2.promisify(fs$j.access);
...
async function send$1 (ctx, path, opts = {}) {
  ...
  // options
  debug$3('send "%s" %j', path, opts);
  const root = opts.root ? normalize(resolve$1(opts.root)) : '';
  const trailingSlash = path[path.length - 1] === '/';
  path = path.substr(parse$2(path).root.length);
  ...
  ctx.body = fs$j.createReadStream(path);
...
const debug$2 = src.exports('koa-static');
...
const childProcess = require$$1__default$3["default"];
const path$c = require$$1__default$1["default"];
const fs$i = fs__default["default"];
...
const debug$1 = src$1.exports('koa-router');
...
Router.prototype.register = function (path, methods, middleware, opts) {
  ...
  debug$1('defined route %s %s', route.methods, route.path);
 ...
 * @api public
 */
var koaCors = function getMiddleware(options) {
...
const getFiles = (input,options={},paths=[]) => {
    const isDir = fs__default["default"].lstatSync(input).isDirectory();
    if (isDir) {
        const files = fs__default["default"].readdirSync(input);
        files.map(file=>{
            if (!options.exclude.includes(file)) {
                getFiles(require$$1__default$1["default"].resolve(input, file), options, paths);
            }
        });
    }
    else {
...
var Stream = require$$2__default["default"].Stream;
...
const getCoverName = (file) => {
    const info = require$$1__default$1["default"].parse(file);
    return `${info.dir}${require$$1__default$1["default"].sep}${info.name}_cover${info.ext}`
};
...
const compressFiles = async (files)=>{
    for(let file of files){
        ...
        if(lib.existsSync(cover) || file.includes('cover')){
            continue;
        }
        sharp__default["default"](file).resize(100).toFile(cover);
    }
};
...
const basePath = require$$4.cwd();
const __dirname$1 = decodeURIComponent(require$$1__default$1["default"].dirname(new URL((typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('server.cjs', document.baseURI).href))).pathname)).replace('/','');
console.log('----import_meta---',({ url: (typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('server.cjs', document.baseURI).href)) }));
console.log('----dirname---',__dirname$1);
...
const { WLAN } = require$$0$2.networkInterfaces();
...
const filter = (file)=>{
  return ['.bmp', '.jpg', '.png', '.tif', '.gif', '.webp', '.jfif', '.jpeg'].includes(require$$1__default$1["default"].extname(file))
};
...
const fileList = files.map(file => {
  if (filter(file)) {
    return {
      name: require$$1__default$1["default"].relative(basePath,file),
      url: `http://${net.address}:${commandOptions.port}/${require$$1__default$1["default"].relative(basePath, file) }`,
      cover: `http://${net.address}:${commandOptions.port}/${require$$1__default$1["default"].relative(basePath, getCoverName(file))}`,
    };
  }
}).filter(file => file);
...
fs__default["default"].writeFileSync('./files.json', JSON.stringify(fileList,null,4));
if (net) {
  ...
  const viewer_path = require$$1__default$1["default"].resolve(__dirname$1,'viewer');  
